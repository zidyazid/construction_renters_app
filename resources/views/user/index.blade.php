...
<div class="container px-5 py-5 bg-white rounded">
    ...
    {{-- <a href="{{ route('suratPernyataan') }}" class="btn btn-danger my-5">Download surat</a> --}}
    <form action="{{ route('store.transaction') }}" method="post">
        @csrf
        <div class="form-group mt-3">
        <label for="nik">Nik Penyewa</label>
        <input type="text" readonly class="form-control" id="nik" name="nik" value="{{ $nikUser[0]["nik"] }}" placeholder="Nama Perusahaan">
            @if($errors->has('nik'))
